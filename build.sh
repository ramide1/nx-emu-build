...
if [[ $(uname -s) == "Darwin" && $(uname -m) == "arm64" ]]; then
    ...
    docker run --platform=linux/amd64 --rm -it -v ./outputs:/root/outputs --privileged ramide1/nx-emu-build:$image_version
else
    docker build -t ramide1/nx-emu-build:$image_version -f Dockerfile_$image_version .
    docker run --rm -it -v ./outputs:/root/outputs --privileged ramide1/nx-emu-build:$image_version
fi
